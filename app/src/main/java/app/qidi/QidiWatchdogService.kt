...
class QidiWatchdogService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        if (watchdogTask?.isDone != false) {
            watchdogTask = executor.scheduleWithFixedDelay(::checkProtectedApps, 0, CHECK_INTERVAL_SECONDS, TimeUnit.SECONDS)
        }
        executor.execute(::checkProtectedApps)
        return START_STICKY
    }
    ...
    private fun checkProtectedApps() {
        ...
        packages.forEach { packageName ->
            ...
            if (state == "running") return@forEach
            if (state != "stopped") {
            ...
            if (updatedState == "running") restarted.add(packageName) else failed.add(packageName)
        }
